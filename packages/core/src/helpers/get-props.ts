...
const getProhibitedKeywordRE = (target: Target) => new RegExp(
  '\\b' +
    (
      ...
      'key,slot,slot-scope,is,' +
      // React handles `ref` as a `forwardRef` properly, but should be included in Vue from the same list as the above line
      target === 'react' ? '' : 'ref' +
 ...
 */
export const getProps = (json: MitosisComponent, target: Target) => {
  const prohibitedKeywordRE = getProhibitedKeywordRE(target);
