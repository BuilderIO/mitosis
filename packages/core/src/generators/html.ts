    ...
    let str = `
      ...
      ${html}\`;
        ...
        render() {
          ...
          const values = Array.from(this.querySelectorAll('[data-dom-state]')).map(el => {
            return {
          ...
          // hydrate input state
          Array.from(this.querySelectorAll('[data-dom-state]')).map((el, index) => {
            const prev = values.find((prev) => el.dataset.domState === prev.id);
            if (prev) {
              el.value = prev.value
              if (prev.active) {
                 el.focus();
