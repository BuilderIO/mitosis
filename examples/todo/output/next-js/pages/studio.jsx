export default function MyComponent(props) {
  return (
    <>
      <style jsx>{`
        .div-1 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-1 {
            padding-left: 20px;
            padding-right: 20px;
            padding-bottom: px;
          }
        }
        .div-2 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-3 {
          padding-top: 0px;
          margin-top: 20px;
          position: relative;
          width: 1400px;
          margin-left: auto;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-3 {
            max-width: 1883px;
            margin-right: 0px;
            margin-left: 0px;
            width: 100%;
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-4 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-4 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-5 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-5 {
            width: 100%;
          }
        }
        .div-6 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          margin-bottom: auto;
          padding-left: 10px;
        }
        .div-7 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          height: auto;
          width: auto;
          border-radius: 30px;
          background: linear-gradient(269.81deg, #45b7cf 0.17%, #207593 99.84%);
          align-self: center;
          flex-grow: 0;
          padding-top: 5px;
          padding-bottom: 5px;
          padding-left: 20px;
          padding-right: 20px;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-7 {
            padding-top: px;
          }
        }
        @media (max-width: 640px) {
          .div-7 {
            width: auto;
            height: auto;
            margin-top: 0px;
            align-self: center;
            flex-grow: 0;
          }
        }
        .div-8 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          line-height: normal;
          height: auto;
          text-align: left;
          color: rgba(244, 243, 233, 1);
          font-size: 14px;
          margin-left: auto;
          margin-right: auto;
          margin-bottom: auto;
          letter-spacing: 0.18em;
          font-family: Basic Sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-8 {
            text-align: center;
          }
        }
        @media (max-width: 640px) {
          .div-8 {
            font-size: 12px;
            text-align: left;
          }
        }
        .div-9 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          padding-left: 0px;
          margin-right: 0px;
        }
        @media (max-width: 991px) {
          .div-9 {
            margin-top: 20px;
            padding-left: 0px;
            margin-right: -0.12px;
          }
        }
        @media (max-width: 640px) {
          .div-9 {
            margin-top: 0px;
          }
        }
        .div-10 {
          display: flex;
          flex-direction: column;
          position: relative;
          line-height: normal;
          height: auto;
          text-align: left;
          color: rgba(14, 32, 26, 1);
          font-size: 56px;
          font-weight: 400;
          margin-right: auto;
          letter-spacing: -0.03em;
          font-family: basic-sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-10 {
            font-size: 30px;
            text-align: left;
            margin-top: 20px;
            width: auto;
            align-self: left;
            margin-right: 0px;
            margin-left: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-10 {
            font-size: 27px;
            margin-top: 20px;
            width: 100%;
            align-self: stretch;
            padding-right: 0px;
          }
        }
        .div-11 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 37px;
          height: auto;
          text-align: left;
          color: rgba(14, 22, 36, 0.6);
          font-size: 18px;
          font-weight: 400;
          padding-left: 0px;
          padding-bottom: 0px;
          padding-right: 0px;
          width: 100%;
          align-self: stretch;
          max-width: 780px;
          margin-left: 0px;
          margin-right: 0px;
          line-height: 32px;
          font-family: basic-sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-11 {
            text-align: left;
            max-height: none;
            margin-left: auto;
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-11 {
            font-size: 17px;
            width: auto;
            align-self: stretch;
            margin-top: 30px;
          }
        }
        .div-12 {
          display: flex;
          flex-direction: row;
          position: relative;
          margin-top: 20px;
          padding-bottom: 0px;
          width: auto;
          align-self: center;
          margin-right: auto;
        }
        @media (max-width: 640px) {
          .div-12 {
            margin-top: 30px;
            width: 100%;
            align-items: center;
          }
        }
        .a-1 {
          display: flex;
          flex-direction: column;
          position: relative;
          height: auto;
          border-radius: 100px;
          background-image: linear-gradient(
            to right,
            #e07748,
            #bd3827,
            #45b7cf,
            #207593
          );
          width: auto;
          cursor: pointer;
          pointer-events: auto;
          margin-right: 20px;
          flex-grow: 0;
          padding-left: 20px;
          padding-right: 20px;
          align-self: center;
        }
        @media (max-width: 640px) {
          .a-1 {
            margin-right: auto;
            width: auto;
            flex-grow: 0;
          }
        }
        .div-13 {
          display: flex;
          flex-direction: column;
          position: relative;
          appearance: none;
          padding-top: 15px;
          padding-bottom: 15px;
          padding-left: 45px;
          padding-right: 45px;
          color: white;
          border-radius: 50px;
          text-align: center;
          cursor: pointer;
          height: auto;
          margin-left: auto;
          margin-right: auto;
          background-image: linear-gradient(to right, #1e231c, #364b34);
          font-size: 24px;
          line-height: 29px;
          font-family: basic-sans, sans-serif;
          flex-grow: 0;
        }
        @media (max-width: 640px) {
          .div-13 {
            font-size: 16px;
            padding-top: 10px;
            padding-left: 15px;
            padding-right: 15px;
            padding-bottom: 10px;
          }
        }
        .div-14 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-14 {
            width: 100%;
          }
        }
        .div-15 {
          display: flex;
          flex-direction: column;
          position: relative;
          min-height: 560px;
          margin-top: auto;
          margin-bottom: auto;
          padding-right: 25px;
          margin-left: 20px;
        }
        @media (max-width: 991px) {
          .div-15 {
            padding-right: 0px;
            align-items: center;
            padding-bottom: 0px;
            margin-top: 40px;
            min-height: 200px;
            margin-left: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-15 {
            justify-content: flex-start;
            margin-left: 0px;
            margin-top: 10px;
            padding-top: 55px;
          }
        }
        .div-16 {
          display: flex;
          flex-direction: column;
          position: relative;
          min-width: 20px;
          overflow: hidden;
          padding-bottom: 0px;
        }
        @media (max-width: 991px) {
          .div-16 {
            margin-top: 0px;
            height: auto;
            padding-bottom: 0px;
            flex-grow: 0;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-16 {
            height: auto;
            margin-top: 20px;
            flex-grow: 0;
          }
        }
        .div-17 {
          position: relative;
        }
        .img-1 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-18 {
          width: 100%;
          padding-top: 72.32%;
          pointer-events: none;
          font-size: 0;
        }
        .div-19 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: -1px;
          height: auto;
          padding-bottom: 30px;
          padding-top: 30px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
          background-color: rgba(244, 243, 233, 1);
        }
        @media (max-width: 991px) {
          .div-19 {
            padding-left: 20px;
            padding-right: 20px;
            padding-bottom: 30px;
            content-visibility: auto;
            contain-intrinsic-size: 274px;
          }
        }
        @media (max-width: 640px) {
          .div-19 {
            padding-left: 20px;
            padding-right: 20px;
            padding-bottom: 0px;
          }
        }
        .div-20 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          line-height: 38px;
          height: auto;
          text-align: center;
          font-weight: 400;
          font-size: 32px;
          width: 70%;
          margin-left: auto;
          margin-right: auto;
          font-family: Basic Sans, sans-serif;
          color: rgba(14, 32, 26, 1);
        }
        @media (max-width: 991px) {
          .div-20 {
            width: 100%;
            font-size: 30px;
          }
        }
        @media (max-width: 640px) {
          .div-20 {
            font-size: 24px;
            padding-left: 0px;
            padding-right: 0px;
            margin-top: 0px;
          }
        }
        .div-21 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 3px;
          padding-bottom: 24px;
          margin-left: auto;
          margin-right: auto;
          width: 50%;
        }
        @media (max-width: 991px) {
          .div-21 {
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-21 {
            width: 100%;
            content-visibility: auto;
            contain-intrinsic-size: 279px;
          }
        }
        .div-22 {
          display: flex;
        }
        @media (max-width: 639px) {
          .div-22 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-23 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(33.333333333333336% - 13.333333333333334px);
          margin-left: 0px;
        }
        @media (max-width: 639px) {
          .div-23 {
            width: 100%;
          }
        }
        .div-24 {
          display: flex;
          flex-direction: row;
          position: relative;
          margin-top: 20px;
          padding-bottom: 30px;
          align-items: center;
          justify-content: center;
        }
        @media (max-width: 640px) {
          .div-24 {
            width: 100%;
          }
        }
        .div-25 {
          display: flex;
          flex-direction: column;
          position: relative;
        }
        .div-26 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-left: 20px;
          line-height: normal;
          height: auto;
          text-align: center;
          color: rgba(224, 119, 72, 1);
          font-size: 14px;
          font-weight: 600;
          font-family: basic-sans, sans-serif;
        }
        @media (max-width: 640px) {
          .div-26 {
            margin-left: 10px;
          }
        }
        .div-27 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(33.333333333333336% - 13.333333333333334px);
          margin-left: 20px;
        }
        @media (max-width: 639px) {
          .div-27 {
            width: 100%;
          }
        }
        .div-28 {
          display: flex;
          flex-direction: row;
          position: relative;
          margin-top: 20px;
          padding-bottom: 30px;
          align-items: center;
          justify-content: center;
        }
        @media (max-width: 640px) {
          .div-28 {
            width: 100%;
          }
        }
        .div-29 {
          display: flex;
          flex-direction: column;
          position: relative;
        }
        .div-30 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-left: 20px;
          line-height: normal;
          height: auto;
          text-align: center;
          color: rgba(69, 183, 207, 1);
          font-size: 14px;
          font-weight: 600;
          font-family: basic-sans, sans-serif;
        }
        @media (max-width: 640px) {
          .div-30 {
            margin-left: 10px;
          }
        }
        .div-31 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(33.333333333333336% - 13.333333333333334px);
          margin-left: 20px;
        }
        @media (max-width: 639px) {
          .div-31 {
            width: 100%;
          }
        }
        .div-32 {
          display: flex;
          flex-direction: row;
          position: relative;
          margin-top: 20px;
          padding-bottom: 30px;
          align-items: center;
          justify-content: center;
        }
        @media (max-width: 640px) {
          .div-32 {
            width: 100%;
          }
        }
        .div-33 {
          display: flex;
          flex-direction: column;
          position: relative;
        }
        .div-34 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-left: 20px;
          line-height: normal;
          height: auto;
          text-align: center;
          color: rgba(54, 92, 66, 1);
          font-size: 14px;
          font-weight: 600;
          font-family: basic-sans, sans-serif;
        }
        @media (max-width: 640px) {
          .div-34 {
            margin-left: 10px;
          }
        }
        .div-35 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-35 {
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-36 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-37 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          width: auto;
          padding-bottom: 30px;
          align-self: center;
          margin-bottom: 0px;
          max-width: 1400px;
          padding-top: 30px;
          height: auto;
          flex-grow: 0;
        }
        @media (max-width: 991px) {
          .div-37 {
            padding-bottom: 30px;
            margin-top: 0px;
            margin-bottom: 0px;
            padding-top: 0px;
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-37 {
            padding-bottom: 0px;
            width: 100%;
            align-self: stretch;
          }
        }
        .div-38 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-38 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-39 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-39 {
            width: 100%;
          }
        }
        .div-40 {
          display: none;
          flex-direction: column;
          position: relative;
          margin-top: -0.5px;
          line-height: normal;
          height: auto;
          text-align: left;
          font-size: 40px;
          font-weight: 600;
          margin-right: auto;
          z-index: 5;
          background-image: linear-gradient(180deg, white 60%, #d3d3d3 40%);
          padding-left: 0px;
          margin-left: 100px;
        }
        @media (max-width: 991px) {
          .div-40 {
            margin-left: auto;
            margin-right: auto;
          }
        }
        @media (max-width: 640px) {
          .div-40 {
            margin-top: 23px;
            font-size: 36px;
            margin-right: auto;
            text-align: left;
            margin-left: auto;
          }
        }
        .div-41 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          height: auto;
          padding-bottom: 0px;
          width: auto;
          z-index: 1;
          align-self: center;
          flex-grow: 0;
          margin-bottom: auto;
        }
        @media (max-width: 991px) {
          .div-41 {
            margin-left: 0px;
            height: auto;
            flex-grow: 0;
            z-index: 15;
            padding-left: 0px;
            margin-top: 0px;
            padding-top: 60px;
          }
        }
        @media (max-width: 640px) {
          .div-41 {
            padding-bottom: 0px;
            padding-left: 0px;
            padding-top: 0px;
          }
        }
        .div-42 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          height: auto;
          width: auto;
          border-radius: 30px;
          background: linear-gradient(270deg, #365c42 0%, #0e201a 100%);
          align-self: center;
          flex-grow: 0;
          padding-top: 5px;
          padding-bottom: 5px;
          padding-left: 20px;
          padding-right: 20px;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-42 {
            padding-top: px;
          }
        }
        @media (max-width: 640px) {
          .div-42 {
            width: auto;
            height: auto;
            margin-top: 20px;
            align-self: center;
            flex-grow: 0;
            padding-top: px;
          }
        }
        .div-43 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          line-height: normal;
          height: auto;
          text-align: left;
          color: rgba(244, 243, 233, 1);
          font-size: 14px;
          margin-left: auto;
          margin-right: auto;
          margin-bottom: auto;
          letter-spacing: 0.18em;
          font-family: Basic Sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-43 {
            text-align: center;
          }
        }
        @media (max-width: 640px) {
          .div-43 {
            font-size: 12px;
            text-align: left;
          }
        }
        .div-44 {
          display: flex;
          flex-direction: column;
          position: relative;
          text-align: left;
          line-height: 54px;
          height: auto;
          font-weight: 400;
          font-size: 54px;
          padding-left: 0px;
          width: 100%;
          margin-top: 10px;
          font-family: Basic Sans, sans-serif;
          color: rgba(14, 32, 26, 1);
          letter-spacing: -0.03em;
        }
        @media (max-width: 991px) {
          .div-44 {
            text-align: left;
            font-size: 55px;
            display: flex;
          }
        }
        @media (max-width: 640px) {
          .div-44 {
            font-size: 32px;
            display: flex;
          }
        }
        .div-45 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 10px;
          text-align: left;
          line-height: 1.5em;
          font-weight: 400;
          font-size: 18px;
          width: 90%;
          margin-bottom: auto;
          z-index: 13;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-45 {
            width: 90%;
            margin-right: auto;
            font-size: 18px;
          }
        }
        @media (max-width: 640px) {
          .div-45 {
            font-size: 16px;
            margin-bottom: 0;
            width: 100%;
            margin-top: 20px;
          }
        }
        .div-46 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-46 {
            width: 100%;
          }
        }
        .div-47 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: -61px;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
          z-index: 2;
          margin-bottom: -90px;
          margin-left: -70px;
          margin-right: -40px;
        }
        @media (max-width: 991px) {
          .div-47 {
            width: 100%;
            height: auto;
            z-index: 5;
            display: flex;
            padding-bottom: 0px;
            margin-top: 0px;
            margin-left: auto;
            flex-grow: 1;
            margin-right: auto;
          }
        }
        @media (max-width: 640px) {
          .div-47 {
            width: 100%;
            position: relative;
            padding-bottom: 0px;
            display: flex;
            margin-bottom: 0px;
          }
        }
        .div-48 {
          position: relative;
        }
        .img-2 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-49 {
          width: 100%;
          padding-top: 70.16%;
          pointer-events: none;
          font-size: 0;
        }
        .div-50 {
          display: flex;
          flex-direction: column;
          position: relative;
          padding-left: 20px;
          padding-right: 20px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
          background-color: rgba(244, 243, 233, 1);
          font-size: 18px;
          color: rgba(14, 32, 26, 1);
          content-visibility: auto;
          contain-intrinsic-size: 9775px;
        }
        @media (max-width: 991px) {
          .div-50 {
            padding-top: 0px;
            content-visibility: auto;
            contain-intrinsic-size: 13700px;
          }
        }
        @media (max-width: 640px) {
          .div-50 {
            content-visibility: auto;
            contain-intrinsic-size: 10919px;
          }
        }
        .div-51 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-52 {
          display: flex;
          flex-direction: column;
          position: relative;
          line-height: normal;
          height: auto;
          text-align: center;
          color: rgba(14, 32, 26, 1);
          margin-bottom: auto;
          font-size: 72px;
          font-family: Basic Sans, sans-serif;
          padding-top: 60px;
          padding-bottom: 30px;
        }
        @media (max-width: 991px) {
          .div-52 {
            font-size: 44px;
            padding-top: 30px;
          }
        }
        @media (max-width: 640px) {
          .div-52 {
            font-size: 18px;
            width: 100%;
            margin-bottom: auto;
            margin-top: 25px;
          }
        }
        .div-53 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-53 {
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-54 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-55 {
          display: flex;
          flex-direction: column;
          position: relative;
          width: 100%;
          align-self: center;
          max-width: 1400px;
          padding-top: 30px;
          padding-bottom: 30px;
        }
        @media (max-width: 991px) {
          .div-55 {
            margin-top: 0px;
            padding-top: 30px;
            padding-bottom: 30px;
            width: 100%;
          }
        }
        .div-56 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-56 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-57 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-57 {
            width: 100%;
          }
        }
        .div-58 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          height: auto;
          padding-bottom: 30px;
          margin-bottom: auto;
        }
        @media (max-width: 991px) {
          .div-58 {
            margin-left: 0px;
            padding-left: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-58 {
            margin-top: 0px;
            padding-left: 0px;
          }
        }
        .div-59 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 60px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 54px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
          letter-spacing: -0.03em;
          width: 100%;
          padding-right: 10px;
        }
        @media (max-width: 991px) {
          .div-59 {
            margin-right: auto;
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-59 {
            font-size: 32px;
            margin-top: 0px;
            padding-right: 0px;
          }
        }
        .div-60 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 18px;
          font-weight: 400;
          width: 70%;
          align-self: center;
          margin-right: auto;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-60 {
            margin-right: auto;
            margin-top: 20px;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-60 {
            font-size: 16px;
            width: 100%;
          }
        }
        .div-61 {
          display: flex;
          flex-direction: row;
          position: relative;
          margin-top: 20px;
          padding-bottom: 30px;
        }
        @media (max-width: 991px) {
          .div-61 {
            align-items: center;
          }
        }
        @media (max-width: 640px) {
          .div-61 {
            padding-bottom: 0px;
          }
        }
        .div-62 {
          display: flex;
          flex-direction: column;
          position: relative;
          line-height: normal;
          height: auto;
          text-align: center;
          font-size: 32px;
          color: rgba(224, 119, 72, 1);
          font-family: basic-sans, sans-serif;
          cursor: pointer;
          pointer-events: auto;
        }
        @media (max-width: 991px) {
          .div-62 {
            font-size: 24px;
            margin-top: auto;
            margin-bottom: auto;
          }
        }
        @media (max-width: 640px) {
          .div-62 {
            font-size: 16px;
          }
        }
        .div-63 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-left: 20px;
          width: 36px;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
          height: 46px;
          margin-top: auto;
          margin-bottom: auto;
        }
        @media (max-width: 991px) {
          .div-63 {
            width: 30px;
            height: 30px;
          }
        }
        @media (max-width: 640px) {
          .div-63 {
            width: 24px;
            height: 24px;
            margin-left: 10px;
          }
        }
        .div-64 {
          position: relative;
        }
        .img-3 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-65 {
          width: 100%;
          padding-top: 120%;
          pointer-events: none;
          font-size: 0;
        }
        .div-66 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-66 {
            width: 100%;
          }
        }
        .div-67 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
        }
        @media (max-width: 991px) {
          .div-67 {
            margin-top: 0px;
          }
        }
        .div-68 {
          position: relative;
        }
        .img-4 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-69 {
          width: 100%;
          padding-top: 80.16%;
          pointer-events: none;
          font-size: 0;
        }
        .div-70 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        .div-71 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-72 {
          display: flex;
          flex-direction: column;
          position: relative;
          width: 100%;
          padding-bottom: 30px;
          margin-top: px;
          align-self: center;
          max-width: 1400px;
          margin-left: px;
          padding-top: 30px;
        }
        @media (max-width: 991px) {
          .div-72 {
            padding-bottom: 30px;
            margin-top: 0px;
            padding-top: 30px;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-72 {
            margin-top: 0px;
          }
        }
        .div-73 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-73 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-74 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-74 {
            width: 100%;
          }
        }
        .div-75 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: center;
          line-height: normal;
          height: auto;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
        }
        @media (max-width: 991px) {
          .div-75 {
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-75 {
            margin-top: 0px;
          }
        }
        .div-76 {
          position: relative;
        }
        .img-5 {
          object-fit: contain;
          object-position: top;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-77 {
          width: 100%;
          padding-top: 80.16%;
          pointer-events: none;
          font-size: 0;
        }
        .div-78 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-78 {
            width: 100%;
          }
        }
        .div-79 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          height: auto;
          padding-bottom: 30px;
          margin-bottom: auto;
          width: 90%;
        }
        @media (max-width: 991px) {
          .div-79 {
            margin-top: 0px;
            padding-left: 0px;
            padding-bottom: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-79 {
            padding-left: 0px;
            padding-bottom: 0px;
            width: 100%;
          }
        }
        .div-80 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 60px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 54px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
          letter-spacing: -0.03em;
          width: 100%;
        }
        @media (max-width: 991px) {
          .div-80 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-80 {
            font-size: 32px;
          }
        }
        .div-81 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 18px;
          font-weight: 400;
          width: 70%;
          align-self: center;
          margin-right: auto;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-81 {
            margin-right: auto;
            margin-top: 20px;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-81 {
            font-size: 16px;
            width: 100%;
          }
        }
        .div-82 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-82 {
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-83 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-84 {
          display: flex;
          flex-direction: column;
          position: relative;
          width: 100%;
          align-self: center;
          max-width: 1400px;
          padding-top: 30px;
          padding-bottom: 30px;
        }
        @media (max-width: 991px) {
          .div-84 {
            margin-top: 0px;
            padding-top: 30px;
            padding-bottom: 30px;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-84 {
            margin-top: 0px;
          }
        }
        .div-85 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-85 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-86 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-86 {
            width: 100%;
          }
        }
        .div-87 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          height: auto;
          padding-bottom: 30px;
          margin-left: 0px;
          margin-bottom: auto;
        }
        @media (max-width: 991px) {
          .div-87 {
            margin-left: 0px;
            padding-left: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-87 {
            margin-top: 20px;
            padding-left: 0px;
          }
        }
        .div-88 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 60px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 54px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
          letter-spacing: -0.03em;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-88 {
            margin-right: auto;
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-88 {
            font-size: 32px;
            margin-top: 0px;
            width: 100%;
          }
        }
        .div-89 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 18px;
          font-weight: 400;
          align-self: center;
          margin-right: auto;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-89 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-89 {
            font-size: 16px;
            width: 100%;
          }
        }
        .div-90 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-90 {
            width: 100%;
          }
        }
        .div-91 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
        }
        @media (max-width: 991px) {
          .div-91 {
            margin-top: 0px;
          }
        }
        .div-92 {
          position: relative;
        }
        .img-6 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-93 {
          width: 100%;
          padding-top: 80.16%;
          pointer-events: none;
          font-size: 0;
        }
        .div-94 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-94 {
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-95 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-96 {
          position: relative;
          margin-top: 0px;
          width: 100%;
          padding-top: 30px;
          padding-bottom: 30px;
          max-width: 1400px;
          align-self: center;
          display: flex;
          flex-direction: column;
        }
        @media (max-width: 991px) {
          .div-96 {
            margin-top: 30px;
            padding-top: 0px;
            padding-bottom: 30px;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-96 {
            padding-bottom: 30px;
            margin-top: 0px;
            padding-top: 30px;
          }
        }
        .div-97 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-97 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-98 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-98 {
            width: 100%;
          }
        }
        .div-99 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          height: auto;
          padding-bottom: 30px;
        }
        @media (max-width: 991px) {
          .div-99 {
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-99 {
            margin-top: 0px;
            padding-bottom: 0px;
          }
        }
        .div-100 {
          display: flex;
          flex-direction: column;
          position: relative;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
        }
        @media (max-width: 991px) {
          .div-100 {
            height: auto;
            flex-grow: 0;
          }
        }
        .div-101 {
          position: relative;
        }
        .img-7 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-102 {
          width: 100%;
          padding-top: 80.16%;
          pointer-events: none;
          font-size: 0;
        }
        .div-103 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-103 {
            width: 100%;
          }
        }
        .div-104 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          height: auto;
          padding-bottom: 30px;
          margin-bottom: auto;
          width: 90%;
        }
        @media (max-width: 991px) {
          .div-104 {
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-104 {
            margin-top: 0px;
            padding-bottom: 0px;
          }
        }
        .div-105 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 60px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 54px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
          letter-spacing: -0.03em;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-105 {
            margin-right: auto;
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-105 {
            font-size: 32px;
            margin-top: 20px;
            width: 100%;
          }
        }
        .div-106 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 18px;
          font-weight: 400;
          align-self: center;
          margin-right: auto;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-106 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-106 {
            font-size: 16px;
            width: 100%;
          }
        }
        .a-2 {
          display: flex;
          position: relative;
          margin-top: 20px;
          height: auto;
          padding-bottom: 0px;
          justify-content: center;
          margin-left: auto;
          margin-right: auto;
          cursor: pointer;
          pointer-events: auto;
        }
        .div-107 {
          display: flex;
          flex-direction: row;
          position: relative;
          margin-top: 20px;
          padding-bottom: 0px;
          justify-content: center;
          align-items: center;
        }
        .div-108 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-108 {
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-109 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .a-3 {
          display: flex;
          flex-direction: column;
          position: relative;
          height: auto;
          border-radius: 100px;
          background-image: linear-gradient(
            to right,
            #e07748,
            #bd3827,
            #45b7cf,
            #207593
          );
          width: auto;
          cursor: pointer;
          pointer-events: auto;
          margin-right: 20px;
          flex-grow: 0;
          padding-left: 20px;
          padding-right: 20px;
          align-self: center;
        }
        @media (max-width: 640px) {
          .a-3 {
            margin-left: auto;
            margin-right: auto;
            width: auto;
            flex-grow: 0;
          }
        }
        .div-110 {
          display: flex;
          flex-direction: column;
          position: relative;
          appearance: none;
          padding-top: 15px;
          padding-bottom: 15px;
          padding-left: 45px;
          padding-right: 45px;
          color: rgba(244, 243, 233, 1);
          border-radius: 50px;
          text-align: center;
          cursor: pointer;
          height: auto;
          margin-left: auto;
          margin-right: auto;
          background-image: linear-gradient(to right, #1e231c, #364b34);
          font-size: 24px;
          line-height: 29px;
          font-family: Basic Sans, sans-serif;
          flex-grow: 0;
        }
        @media (max-width: 640px) {
          .div-110 {
            font-size: 16px;
            padding-top: 10px;
            padding-left: 15px;
            padding-right: 15px;
            padding-bottom: 10px;
          }
        }
        .div-111 {
          display: flex;
          flex-direction: column;
          position: relative;
          width: 100%;
          flex-grow: 0;
          padding-bottom: 30px;
          background-color: rgba(244, 243, 233, 1);
          height: auto;
          max-width: 1400px;
          align-self: center;
          margin-top: 0px;
          padding-top: 30px;
        }
        @media (max-width: 991px) {
          .div-111 {
            height: auto;
            flex-grow: 0;
            padding-bottom: 30px;
            margin-top: 20px;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-111 {
            margin-top: 0px;
          }
        }
        .div-112 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-112 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-113 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-113 {
            width: 100%;
          }
        }
        .div-114 {
          display: flex;
          flex-direction: column;
          position: relative;
          height: auto;
          padding-bottom: 30px;
          background-color: rgba(244, 243, 233, 1);
          margin-top: auto;
          margin-bottom: auto;
        }
        @media (max-width: 991px) {
          .div-114 {
            margin-left: 0px;
            padding-left: 0px;
            padding-bottom: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-114 {
            padding-left: 0px;
            margin-top: 20px;
          }
        }
        .div-115 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          height: auto;
          width: auto;
          border-radius: 30px;
          background: linear-gradient(269.81deg, #45b7cf 0.17%, #207593 99.84%);
          align-self: center;
          flex-grow: 0;
          padding-top: 5px;
          padding-bottom: 5px;
          padding-left: 20px;
          padding-right: 20px;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-115 {
            padding-top: px;
          }
        }
        @media (max-width: 640px) {
          .div-115 {
            width: auto;
            height: auto;
            margin-top: 0px;
            padding-left: 20px;
            align-self: center;
            flex-grow: 0;
          }
        }
        .div-116 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          line-height: normal;
          height: auto;
          text-align: left;
          color: rgba(244, 243, 233, 1);
          font-size: 14px;
          margin-left: auto;
          margin-right: auto;
          margin-bottom: auto;
          letter-spacing: 0.18em;
          font-family: Basic Sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-116 {
            text-align: center;
          }
        }
        @media (max-width: 640px) {
          .div-116 {
            font-size: 12px;
            text-align: left;
          }
        }
        .div-117 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 54px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
          letter-spacing: -0.03em;
        }
        @media (max-width: 991px) {
          .div-117 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-117 {
            font-size: 32px;
          }
        }
        .div-118 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 18px;
          font-weight: 400;
          align-self: center;
          margin-right: auto;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-118 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-118 {
            font-size: 16px;
            width: 100%;
          }
        }
        .div-119 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-119 {
            width: 100%;
          }
        }
        .div-120 {
          display: flex;
          flex-direction: column;
          position: relative;
          height: auto;
          padding-bottom: 0px;
        }
        @media (max-width: 991px) {
          .div-120 {
            padding-bottom: 0px;
            height: auto;
            flex-grow: 0;
          }
        }
        .div-121 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
        }
        .div-122 {
          position: relative;
        }
        .img-8 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-123 {
          width: 100%;
          padding-top: 80.16%;
          pointer-events: none;
          font-size: 0;
        }
        .div-124 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 0px;
          padding-right: 0px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100%;
          background-color: linear-gradient(45deg, #f4f3e9 0%, #ffffff 100%);
          height: auto;
          flex-grow: 0;
          max-width: 1400px;
          margin-left: auto;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-124 {
            height: auto;
            flex-grow: 0;
            padding-bottom: 0px;
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-124 {
            padding-top: 0px;
          }
        }
        .div-125 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-126 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
          background-color: rgba(244, 243, 233, 1);
          height: auto;
          flex-grow: 0;
        }
        @media (max-width: 991px) {
          .div-126 {
            height: auto;
            flex-grow: 0;
            padding-bottom: 30px;
            padding-top: 30px;
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-126 {
            padding-bottom: 30px;
            margin-top: 0px;
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-127 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-128 {
          display: flex;
          flex-direction: column;
          position: relative;
          width: 100%;
          flex-grow: 1;
          background-color: rgba(244, 243, 233, 1);
          padding-bottom: 30px;
          max-width: 1400px;
          align-self: center;
          padding-top: 30px;
        }
        @media (max-width: 991px) {
          .div-128 {
            height: auto;
            flex-grow: 0;
            padding-bottom: 0px;
            margin-top: 20px;
            width: 100%;
          }
        }
        @media (max-width: 640px) {
          .div-128 {
            margin-top: 0px;
          }
        }
        .div-129 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-129 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-130 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-130 {
            width: 100%;
          }
        }
        .div-131 {
          display: flex;
          flex-direction: column;
          position: relative;
          height: auto;
          padding-bottom: 30px;
          background-color: rgba(244, 243, 233, 1);
        }
        @media (max-width: 991px) {
          .div-131 {
            margin-left: 0px;
            padding-left: 30px;
            padding-bottom: 55px;
          }
        }
        @media (max-width: 640px) {
          .div-131 {
            padding-bottom: 0px;
            padding-left: 0px;
          }
        }
        .div-132 {
          display: flex;
          flex-direction: column;
          position: relative;
          text-align: center;
          line-height: normal;
          height: auto;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
          flex-grow: 1;
          margin-top: 55px;
        }
        @media (max-width: 991px) {
          .div-132 {
            height: auto;
            flex-grow: 0;
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-132 {
            padding-bottom: px;
            margin-bottom: 30px;
            margin-top: 0px;
          }
        }
        .div-133 {
          position: relative;
        }
        .img-9 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-134 {
          width: 100%;
          padding-top: 80.16%;
          pointer-events: none;
          font-size: 0;
        }
        .div-135 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-135 {
            width: 100%;
          }
        }
        .div-136 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          height: auto;
          padding-bottom: 30px;
          margin-bottom: auto;
        }
        .div-137 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          height: auto;
          width: auto;
          border-radius: 30px;
          background: linear-gradient(270deg, #e07748 0%, #bd3827 100%);
          align-self: center;
          flex-grow: 0;
          padding-top: 5px;
          padding-bottom: 5px;
          padding-left: 20px;
          padding-right: 20px;
          margin-right: auto;
          margin-left: 20px;
        }
        @media (max-width: 991px) {
          .div-137 {
            padding-top: px;
            margin-left: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-137 {
            width: auto;
            height: auto;
            margin-top: 0px;
            align-self: center;
            flex-grow: 0;
          }
        }
        .div-138 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          line-height: normal;
          height: auto;
          text-align: left;
          color: rgba(244, 243, 233, 1);
          font-size: 14px;
          margin-left: auto;
          margin-right: auto;
          margin-bottom: auto;
          letter-spacing: 0.18em;
          font-family: Basic Sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-138 {
            text-align: center;
          }
        }
        @media (max-width: 640px) {
          .div-138 {
            font-size: 12px;
            text-align: left;
          }
        }
        .div-139 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 54px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
          letter-spacing: -0.03em;
        }
        @media (max-width: 991px) {
          .div-139 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-139 {
            font-size: 32px;
          }
        }
        .div-140 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 18px;
          font-weight: 400;
          align-self: center;
          margin-right: auto;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-140 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-140 {
            font-size: 16px;
            width: 100%;
          }
        }
        .div-141 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          padding-left: 40px;
          padding-right: 40px;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-141 {
            padding-left: 20px;
            padding-right: 20px;
          }
        }
        .div-142 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-143 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          max-width: 1400px;
          padding-top: 30px;
          padding-bottom: 30px;
          width: 100%;
          align-self: center;
        }
        @media (max-width: 991px) {
          .div-143 {
            width: 100%;
          }
        }
        .div-144 {
          display: flex;
        }
        @media (max-width: 999px) {
          .div-144 {
            flex-direction: column;
            align-items: stretch;
          }
        }
        .div-145 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 0px;
        }
        @media (max-width: 999px) {
          .div-145 {
            width: 100%;
          }
        }
        .div-146 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          height: auto;
          padding-bottom: 30px;
          margin-bottom: auto;
        }
        @media (max-width: 640px) {
          .div-146 {
            margin-top: 20px;
          }
        }
        .div-147 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          height: auto;
          width: auto;
          border-radius: 30px;
          background: linear-gradient(270deg, #365c42 0%, #0e201a 100%);
          align-self: center;
          flex-grow: 0;
          padding-top: 5px;
          padding-bottom: 5px;
          padding-left: 20px;
          padding-right: 20px;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-147 {
            padding-top: px;
          }
        }
        @media (max-width: 640px) {
          .div-147 {
            width: auto;
            height: auto;
            margin-top: 0px;
            align-self: center;
            flex-grow: 0;
          }
        }
        .div-148 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: auto;
          line-height: normal;
          height: auto;
          text-align: left;
          color: rgba(244, 243, 233, 1);
          font-size: 14px;
          margin-left: auto;
          margin-right: auto;
          margin-bottom: auto;
          letter-spacing: 0.18em;
          font-family: Basic Sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-148 {
            text-align: center;
          }
        }
        @media (max-width: 640px) {
          .div-148 {
            font-size: 12px;
            text-align: left;
          }
        }
        .div-149 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 54px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
          letter-spacing: -0.03em;
        }
        @media (max-width: 991px) {
          .div-149 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-149 {
            font-size: 32px;
          }
        }
        .div-150 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 30px;
          text-align: left;
          line-height: normal;
          height: auto;
          font-size: 18px;
          font-weight: 400;
          align-self: center;
          margin-right: auto;
          color: rgba(14, 32, 26, 0.6);
          font-family: Avenir, sans-serif;
        }
        @media (max-width: 991px) {
          .div-150 {
            margin-right: auto;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-150 {
            font-size: 16px;
            width: 100%;
          }
        }
        .div-151 {
          display: flex;
          flex-direction: column;
          line-height: normal;
          width: calc(50% - 10px);
          margin-left: 20px;
        }
        @media (max-width: 999px) {
          .div-151 {
            width: 100%;
          }
        }
        .div-152 {
          display: flex;
          flex-direction: column;
          position: relative;
          text-align: center;
          line-height: normal;
          height: auto;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
        }
        .div-153 {
          position: relative;
        }
        .img-10 {
          object-fit: contain;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-154 {
          width: 100%;
          padding-top: 70.04048582995948%;
          pointer-events: none;
          font-size: 0;
        }
        .div-155 {
          display: flex;
          flex-direction: column;
          position: relative;
          padding-top: 0px;
          padding-bottom: 0px;
          width: 100%;
          background-color: rgba(244, 243, 233, 1);
          height: auto;
          flex-grow: 0;
          max-width: 1400px;
          margin-left: auto;
          margin-right: auto;
        }
        @media (max-width: 991px) {
          .div-155 {
            height: auto;
            flex-grow: 0;
            margin-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-155 {
            margin-top: 0px;
            height: auto;
            flex-grow: 0;
          }
        }
        .div-156 {
          width: 100%;
          align-self: stretch;
          flex-grow: 1;
          max-width: 1200px;
          display: flex;
          flex-direction: column;
          margin-left: auto;
          margin-right: auto;
        }
        .div-157 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 0px;
          min-height: 20px;
          min-width: 20px;
          overflow: hidden;
          width: 100vw;
          margin-left: calc(50% - 50vw);
        }
        @media (max-width: 640px) {
          .div-157 {
            margin-top: 0px;
          }
        }
        .div-158 {
          position: relative;
        }
        .img-11 {
          object-fit: cover;
          object-position: center;
          position: absolute;
          height: 100%;
          width: 100%;
          top: 0;
          left: 0;
        }
        .div-159 {
          width: 100%;
          padding-top: 70.6%;
          pointer-events: none;
          font-size: 0;
        }
        .div-160 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 94px;
          line-height: normal;
          height: auto;
          text-align: left;
          font-size: 72px;
          font-weight: 400;
          font-family: Basic Sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-160 {
            font-size: 35px;
            margin-top: 20px;
          }
        }
        @media (max-width: 640px) {
          .div-160 {
            font-size: 32px;
            margin-top: 20px;
          }
        }
        .div-161 {
          display: flex;
          flex-direction: column;
          position: relative;
          margin-top: 20px;
          line-height: normal;
          height: auto;
          text-align: left;
          font-size: 24px;
          font-weight: 600;
          width: 74%;
          margin-bottom: 30px;
          font-family: Basic Sans, sans-serif;
        }
        @media (max-width: 991px) {
          .div-161 {
            font-size: 20px;
            width: 90%;
          }
        }
        @media (max-width: 640px) {
          .div-161 {
            width: 100%;
            font-size: 16px;
          }
        }
        .div-162 {
          display: flex;
          position: relative;
          margin-top: 20px;
          height: auto;
          width: 100%;
          background-color: rgba(244, 243, 233, 1);
          padding-bottom: 60px;
          padding-top: 60px;
        }
        @media (max-width: 991px) {
          .div-162 {
            padding-top: 0px;
          }
        }
        @media (max-width: 640px) {
          .div-162 {
            padding-top: 0px;
          }
        }
      `}</style>
      <>
        <div className="div-1">
          <div className="div-2">
            <div className="div-3">
              <div className="builder-columns" className="div-4">
                <div className="builder-column" className="div-5">
                  <div className="div-6">
                    <div className="div-7">
                      <div className="div-8">
                        <p>THEME STUDIO</p>
                      </div>
                    </div>

                    <div className="div-9">
                      <div className="div-10">
                        <p>Create no-code Shopify magic</p>
                      </div>

                      <div className="div-11">
                        <p>
                          Builder Visual CMS
                          <strong> </strong>
                          is the first and only headless CMS with a powerful
                          drag-and-drop visual editor that lets you build and
                          optimize digital experiences with speed and
                          flexibility
                        </p>
                      </div>
                    </div>

                    <div className="div-12">
                      <a
                        href="https://apps.shopify.com/builder-2"
                        className="a-1"
                      >
                        <div className="div-13">
                          <a href="" target="_self">
                            Try for free
                          </a>
                        </div>
                      </a>
                    </div>
                  </div>
                </div>

                <div className="builder-column" className="div-14">
                  <div className="div-15">
                    <div className="div-16">
                      <div className="div-17">
                        <picture>
                          <source
                            srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=1355 1355w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=558 558w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?format=webp&width=762 762w"
                            type="image/webp"
                          />

                          <img
                            loading="lazy"
                            src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=1355"
                            sizes="(max-width: 638px) 88vw, 55vw"
                            srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=1355 1355w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=558 558w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F003add813b3149e9b56c17c2843cebde?width=762 762w"
                            className="img-1"
                          />
                        </picture>

                        <div
                          className="builder-image-sizer"
                          className="div-18"
                        ></div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="div-19">
          <div className="div-20">
            <p>
              Build the e-commerce store of your dreams. No code, no stress, no
              limits.
            </p>
          </div>

          <div className="div-21">
            <div className="builder-columns" className="div-22">
              <div className="builder-column" className="div-23">
                <div className="div-24">
                  <div className="div-25">
                    <div
                      dangerouslySetInnerHTML={{
                        __html:
                          "<svg  width=&quot;30&quot;  height=&quot;30&quot;  viewBox=&quot;0 0 25 24&quot;  fill=&quot;none&quot;  xmlns=&quot;http://www.w3.org/2000/svg&quot; >  <path  fill-rule=&quot;evenodd&quot;  clip-rule=&quot;evenodd&quot;  d=&quot;M1.25 12C1.25 5.7868 6.2868 0.75 12.5 0.75C18.7132 0.75 23.75 5.7868 23.75 12C23.75 18.2132 18.7132 23.25 12.5 23.25C6.2868 23.25 1.25 18.2132 1.25 12ZM21.25 12C21.25 7.16751 17.3325 3.25 12.5 3.25C7.66751 3.25 3.75 7.16751 3.75 12C3.75 16.8325 7.66751 20.75 12.5 20.75C17.3325 20.75 21.25 16.8325 21.25 12Z&quot;  fill=&quot;#E07748&quot;  />  <path  d=&quot;M12.5 5.75C13.1472 5.75 13.6795 6.24187 13.7435 6.87219L13.75 7V11.483L16.3839 14.1161C16.8395 14.5717 16.8699 15.2915 16.475 15.7824L16.3839 15.8839C15.9283 16.3395 15.2085 16.3699 14.7176 15.975L14.6161 15.8839L11.6161 12.8839C11.4152 12.683 11.2897 12.4207 11.258 12.141L11.25 12V7C11.25 6.30964 11.8096 5.75 12.5 5.75Z&quot;  fill=&quot;#E07748&quot;  /> </svg> ",
                      }}
                    ></div>
                  </div>

                  <div className="div-26">
                    <p>SPEND LESS</p>
                  </div>
                </div>
              </div>

              <div className="builder-column" className="div-27">
                <div className="div-28">
                  <div className="div-29">
                    <div
                      dangerouslySetInnerHTML={{
                        __html:
                          "<svg  width=&quot;30&quot;  height=&quot;30&quot;  viewBox=&quot;0 0 25 24&quot;  fill=&quot;none&quot;  xmlns=&quot;http://www.w3.org/2000/svg&quot; >  <path  fill-rule=&quot;evenodd&quot;  clip-rule=&quot;evenodd&quot;  d=&quot;M11.25 21V3.00001C11.25 1.9178 12.5318 1.34693 13.3362 2.07089L23.3362 11.0709C23.8879 11.5674 23.8879 12.4326 23.3362 12.9291L13.3362 21.9291C12.5318 22.6531 11.25 22.0822 11.25 21ZM20.63 12L13.75 5.80701V18.192L20.63 12Z&quot;  fill=&quot;#45B7CF&quot;  />  <path  fill-rule=&quot;evenodd&quot;  clip-rule=&quot;evenodd&quot;  d=&quot;M1.25 21V3.00001C1.25 1.9178 2.5318 1.34693 3.33621 2.07089L13.3362 11.0709C13.8879 11.5674 13.8879 12.4326 13.3362 12.9291L3.33621 21.9291C2.5318 22.6531 1.25 22.0822 1.25 21ZM10.63 12L3.75 5.80701V18.192L10.63 12Z&quot;  fill=&quot;#45B7CF&quot;  /> </svg> ",
                      }}
                    ></div>
                  </div>

                  <div className="div-30">
                    <p>MOVE FASTER</p>
                  </div>
                </div>
              </div>

              <div className="builder-column" className="div-31">
                <div className="div-32">
                  <div className="div-33">
                    <div
                      dangerouslySetInnerHTML={{
                        __html:
                          "<svg  width=&quot;30&quot;  height=&quot;30&quot;  viewBox=&quot;0 0 25 20&quot;  fill=&quot;none&quot;  xmlns=&quot;http://www.w3.org/2000/svg&quot; >  <path  d=&quot;M16.3069 1.62716C16.6604 0.495852 18.2167 0.461155 18.6507 1.50978L18.6973 1.64082L21.43 10.75H23C23.6472 10.75 24.1795 11.2419 24.2435 11.8722L24.25 12C24.25 12.6472 23.7581 13.1795 23.1278 13.2435L23 13.25H20.5C19.9905 13.25 19.5366 12.9415 19.345 12.4782L19.3027 12.3592L17.475 6.269L13.6931 18.3728C13.3589 19.4424 11.9174 19.5509 11.4053 18.6047L11.3462 18.4809L7.396 9.004L5.61785 12.5594C5.42721 12.9403 5.05765 13.195 4.6406 13.2421L4.5 13.25H2C1.30964 13.25 0.75 12.6904 0.75 12C0.75 11.3528 1.24187 10.8205 1.87219 10.7565L2 10.75H3.727L6.38215 5.44565C6.84011 4.5305 8.12375 4.53436 8.59474 5.40063L8.65377 5.5241L12.331 14.347L16.3069 1.62716Z&quot;  fill=&quot;#365C42&quot;  /> </svg> ",
                      }}
                    ></div>
                  </div>

                  <div className="div-34">
                    <p>DO MORE</p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="div-35">
          <div className="div-36">
            <div className="div-37">
              <div className="builder-columns" className="div-38">
                <div className="builder-column" className="div-39">
                  <div className="div-40">
                    <p>How</p>
                  </div>
                  <div className="div-41">
                    <div className="div-42">
                      <div className="div-43">
                        <p>BUILD</p>
                      </div>
                    </div>
                    <div className="div-44">
                      <p>Make it exactly as you want it</p>
                    </div>
                    <div className="div-45">
                      <p>
                        This isn't just another page builder. Our powerful
                        visual editor lets you drag-and-drop to create and
                        customize anything, from header to footer. Start with
                        our templates, import your current pages, or start from
                        scratch to quickly bring your vision to life. (And
                        remember, no developer required!){" "}
                      </p>{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
                <div className="builder-column" className="div-46">
                  {" "}
                  <div className="div-47">
                    {" "}
                    <div className="div-48">
                      {" "}
                      <picture>
                        {" "}
                        <source
                          srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=1020 1020w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=558 558w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?format=webp&width=917 917w"
                          type="image/webp"
                        />{" "}
                        <img
                          loading="lazy"
                          src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=1020"
                          sizes="(max-width: 638px) 88vw, 66vw"
                          srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=1020 1020w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=558 558w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F7d626c4de12b4ca4a80e191551b60c44?width=917 917w"
                          className="img-2"
                        />{" "}
                      </picture>{" "}
                      <div
                        className="builder-image-sizer"
                        className="div-49"
                      ></div>{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
              </div>{" "}
            </div>{" "}
          </div>{" "}
        </div>{" "}
        <div className="div-50">
          {" "}
          <div className="div-51">
            {" "}
            <div className="div-52">
              {" "}
              <p>Endless customization, endless possibilities</p>{" "}
            </div>{" "}
            <div className="div-53">
              {" "}
              <div className="div-54">
                {" "}
                <div className="div-55">
                  {" "}
                  <div className="builder-columns" className="div-56">
                    {" "}
                    <div className="builder-column" className="div-57">
                      {" "}
                      <div className="div-58">
                        {" "}
                        <div className="div-59">
                          {" "}
                          <p>Getting started is easy</p>{" "}
                        </div>{" "}
                        <div className="div-60">
                          {" "}
                          <p>
                            {" "}
                            Get started with our templates, import your own, or
                            start from scratch.{" "}
                          </p>{" "}
                        </div>{" "}
                        <div className="div-61">
                          {" "}
                          <div
                            href="https://apps.shopify.com/builder-2"
                            className="div-62"
                          >
                            {" "}
                            <p>Start Building</p>{" "}
                          </div>{" "}
                          <div className="div-63">
                            {" "}
                            <div className="div-64">
                              {" "}
                              <picture>
                                {" "}
                                <source
                                  srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=36 36w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?format=webp&width=24 24w"
                                  type="image/webp"
                                />{" "}
                                <img
                                  loading="lazy"
                                  src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=36"
                                  sizes="4vw"
                                  srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=36 36w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F4d4d8dd8ab274d28b694cbb3e802f3d8?width=24 24w"
                                  className="img-3"
                                />{" "}
                              </picture>{" "}
                              <div
                                className="builder-image-sizer"
                                className="div-65"
                              ></div>{" "}
                            </div>{" "}
                          </div>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                    <div className="builder-column" className="div-66">
                      {" "}
                      <div className="div-67">
                        {" "}
                        <div className="div-68">
                          {" "}
                          <picture>
                            {" "}
                            <source
                              srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?format=webp&width=817 817w"
                              type="image/webp"
                            />{" "}
                            <img
                              loading="lazy"
                              src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=918"
                              sizes="(max-width: 638px) 94vw, 59vw"
                              srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Fff632f82f24249678e1c4ad44ba2fd71?width=817 817w"
                              className="img-4"
                            />{" "}
                          </picture>{" "}
                          <div
                            className="builder-image-sizer"
                            className="div-69"
                          ></div>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
                <div className="div-70">
                  {" "}
                  <div className="div-71">
                    {" "}
                    <div className="div-72">
                      {" "}
                      <div className="builder-columns" className="div-73">
                        {" "}
                        <div className="builder-column" className="div-74">
                          {" "}
                          <div className="div-75">
                            {" "}
                            <div className="div-76">
                              {" "}
                              <picture>
                                {" "}
                                <source
                                  srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=558 558w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?format=webp&width=675 675w"
                                  type="image/webp"
                                />{" "}
                                <img
                                  loading="lazy"
                                  src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=918"
                                  sizes="(max-width: 638px) 88vw, 49vw"
                                  srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=558 558w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa5d8ab04b724171bf54fdc7018b3ea5?width=675 675w"
                                  className="img-5"
                                />{" "}
                              </picture>{" "}
                              <div
                                className="builder-image-sizer"
                                className="div-77"
                              ></div>{" "}
                            </div>{" "}
                          </div>{" "}
                        </div>{" "}
                        <div className="builder-column" className="div-78">
                          {" "}
                          <div className="div-79">
                            {" "}
                            <div className="div-80">
                              {" "}
                              <p>All the building blocks you need</p>{" "}
                            </div>{" "}
                            <div className="div-81">
                              {" "}
                              <p>
                                {" "}
                                Drag-and-drop it all, from text, images, and
                                videos to products, collections, and
                                add-to-carts.{" "}
                              </p>{" "}
                            </div>{" "}
                          </div>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
              </div>{" "}
            </div>{" "}
            <div className="div-82">
              {" "}
              <div className="div-83">
                {" "}
                <div className="div-84">
                  {" "}
                  <div className="builder-columns" className="div-85">
                    {" "}
                    <div className="builder-column" className="div-86">
                      {" "}
                      <div className="div-87">
                        {" "}
                        <div className="div-88">
                          {" "}
                          <p>Make it your own</p>{" "}
                        </div>{" "}
                        <div className="div-89">
                          {" "}
                          <p>
                            {" "}
                            Completely customize the look and feel of your
                            content and make it come alive with animations.{" "}
                          </p>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                    <div className="builder-column" className="div-90">
                      {" "}
                      <div className="div-91">
                        {" "}
                        <div className="div-92">
                          {" "}
                          <picture>
                            {" "}
                            <source
                              srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?format=webp&width=817 817w"
                              type="image/webp"
                            />{" "}
                            <img
                              loading="lazy"
                              src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=918"
                              sizes="(max-width: 638px) 94vw, 59vw"
                              srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F30c350c0c9fe4578aa7ade21796aa1b8?width=817 817w"
                              className="img-6"
                            />{" "}
                          </picture>{" "}
                          <div
                            className="builder-image-sizer"
                            className="div-93"
                          ></div>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
              </div>{" "}
            </div>{" "}
            <div className="div-94">
              {" "}
              <div className="div-95">
                {" "}
                <div className="div-96">
                  {" "}
                  <div className="builder-columns" className="div-97">
                    {" "}
                    <div className="builder-column" className="div-98">
                      {" "}
                      <div className="div-99">
                        {" "}
                        <div className="div-100">
                          {" "}
                          <div className="div-101">
                            {" "}
                            <picture>
                              {" "}
                              <source
                                srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?format=webp&width=690 690w"
                                type="image/webp"
                              />{" "}
                              <img
                                loading="lazy"
                                src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=918"
                                sizes="(max-width: 638px) 94vw, 50vw"
                                srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F28cf67e4fb5e4ada8391c5bb4d18694e?width=690 690w"
                                className="img-7"
                              />{" "}
                            </picture>{" "}
                            <div
                              className="builder-image-sizer"
                              className="div-102"
                            ></div>{" "}
                          </div>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                    <div className="builder-column" className="div-103">
                      {" "}
                      <div className="div-104">
                        {" "}
                        <div className="div-105">
                          {" "}
                          <p>Build for the entire customer journey</p>{" "}
                        </div>{" "}
                        <div className="div-106">
                          {" "}
                          <p>
                            {" "}
                            Create the optimal end-to-end experience, from
                            high-impact pages (collections, product pages, cart)
                            to the essential touches (pop-ups, slide-ups,
                            announcement bars).{" "}
                          </p>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
              </div>{" "}
            </div>{" "}
            <a
              href="https://apps.shopify.com/builder-2"
              target="_blank"
              className="a-2"
            >
              {" "}
              <div className="div-107"></div>{" "}
            </a>{" "}
            <div className="div-108">
              {" "}
              <div className="div-109">
                {" "}
                <a href="https://apps.shopify.com/builder-2" className="a-3">
                  {" "}
                  <div className="div-110">
                    {" "}
                    <a href="" target="_self">
                      {" "}
                      Try for free{" "}
                    </a>{" "}
                  </div>{" "}
                </a>{" "}
                <div className="div-111">
                  {" "}
                  <div className="builder-columns" className="div-112">
                    {" "}
                    <div className="builder-column" className="div-113">
                      {" "}
                      <div className="div-114">
                        {" "}
                        <div className="div-115">
                          {" "}
                          <div className="div-116">
                            {" "}
                            <p>OPTIMIZE</p>{" "}
                          </div>{" "}
                        </div>{" "}
                        <div className="div-117">
                          {" "}
                          <p>Make sure your content converts</p>{" "}
                        </div>{" "}
                        <div className="div-118">
                          {" "}
                          <p>
                            {" "}
                            With Theme Studio, you can personalize your
                            storefront by creating targeted page-level or
                            content-specific experiences for different customer
                            segments. Perform unlimited A/B tests to understand
                            how and if changes impact funnel performance. Plus,
                            our dashboards tell you just how much money each
                            change made, and allows you to revert any misstep
                            instantly. It's like magic.
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="builder-column" className="div-119">
                      <div className="div-120">
                        <div className="div-121">
                          <div className="div-122">
                            <picture>
                              <source
                                srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?format=webp&width=792 792w"
                                type="image/webp"
                              />

                              <img
                                loading="lazy"
                                src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=918"
                                sizes="(max-width: 638px) 94vw, 57vw"
                                srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Feb326c42e0df4b1ba4479905005f7fde?width=792 792w"
                                className="img-8"
                              />
                            </picture>

                            <div
                              className="builder-image-sizer"
                              className="div-123"
                            ></div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="div-124">
              <div className="div-125"></div>
            </div>
            <div className="div-126">
              <div className="div-127">
                <div className="div-128">
                  <div className="builder-columns" className="div-129">
                    <div className="builder-column" className="div-130">
                      <div className="div-131">
                        <div className="div-132">
                          <div className="div-133">
                            <picture>
                              <source
                                srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?format=webp&width=559 559w"
                                type="image/webp"
                              />

                              <img
                                loading="lazy"
                                src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=918"
                                sizes="(max-width: 638px) 94vw, 40vw"
                                srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F69ac0d8eacdc4e699227588000a4b5f4?width=559 559w"
                                className="img-9"
                              />
                            </picture>

                            <div
                              className="builder-image-sizer"
                              className="div-134"
                            ></div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="builder-column" className="div-135">
                      <div className="div-136">
                        <div className="div-137">
                          <div className="div-138">
                            <p>ANALYZE</p>
                          </div>
                        </div>
                        <div className="div-139">
                          <p>Know what is, and isn’t working</p>
                        </div>
                        <div className="div-140">
                          <p>
                            The best merchants find ways to test, iterate,
                            measure, and then do it better the next time. Theme
                            Studio's analytics suite comes with site, page, and
                            content-level insights, as well as heat maps that
                            show both engagement (nice) and impact on
                            conversation (very nice).{" "}
                          </p>{" "}
                        </div>{" "}
                      </div>{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
                <div className="div-141">
                  {" "}
                  <div className="div-142">
                    {" "}
                    <div className="div-143">
                      {" "}
                      <div className="builder-columns" className="div-144">
                        {" "}
                        <div className="builder-column" className="div-145">
                          {" "}
                          <div className="div-146">
                            {" "}
                            <div className="div-147">
                              {" "}
                              <div className="div-148">
                                {" "}
                                <p>CUSTOMIZE</p>{" "}
                              </div>{" "}
                            </div>{" "}
                            <div className="div-149">
                              {" "}
                              <p>Tons of apps, all out-of-the-box</p>{" "}
                            </div>{" "}
                            <div className="div-150">
                              {" "}
                              <p>
                                {" "}
                                Stop installing an endless number of apps. Theme
                                Studio comes with so much power and so many
                                capabilities, you'll be able to uninstall apps
                                that needlessly slow down your storefront and
                                only keep the ones providing real value.
                              </p>
                            </div>
                          </div>
                        </div>
                        <div className="builder-column" className="div-151">
                          <div className="div-152">
                            <div className="div-153">
                              <picture>
                                <source
                                  srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?format=webp&width=807 807w"
                                  type="image/webp"
                                />

                                <img
                                  loading="lazy"
                                  src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=918"
                                  sizes="(max-width: 638px) 94vw, 58vw"
                                  srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=918 918w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=598 598w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F0d443f6c91734e7ea557debe74bdd63b?width=807 807w"
                                  className="img-10"
                                />
                              </picture>

                              <div
                                className="builder-image-sizer"
                                className="div-154"
                              ></div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="div-155">
              <div className="div-156"></div>
            </div>
            <div className="div-157">
              <div className="div-158">
                <picture>
                  <source
                    srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=638 638w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?format=webp&width=998 998w"
                    type="image/webp"
                  />

                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=2000"
                    sizes="(max-width: 638px) 100vw, 100vw"
                    srcSet="https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=100 100w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=200 200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=400 400w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=800 800w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=1200 1200w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=1600 1600w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=2000 2000w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=638 638w, https://cdn.builder.io/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2Faa89047b9471479a960a994234fd77c8?width=998 998w"
                    className="img-11"
                  />
                </picture>

                <div className="builder-image-sizer" className="div-159"></div>
              </div>
            </div>
            <div className="div-160">
              <p>
                <span className="underline-3">Frequently</span>
                Asked Questions
              </p>
            </div>
            <div className="div-161">
              <p>
                Have a different question about how Builder.io works or the
                pricing plans available? Try getting in touch with one of our
                specialists via live chat.
              </p>
            </div>
            <div className="div-162"></div>
          </div>
        </div>
      </>
    </>
  );
}
